...
(defvar tla--key-apply-from-head ?@)    ; Do something head revision
;;;###autoload
...
(defun tla--prefix-apply-from-here (&rest keys)
  (tla--key-group tla--key-apply-from-here keys))

(defun tla--prefix-apply-from-head (&rest keys)
  (tla--key-group tla--key-apply-from-head keys))
...
(defvar tla--context-map-template
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (tla--prefix-apply-from-here tla--key-help) 'describe-bindings)
...
(defvar tla-bookmarks-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; Fields
    (define-key map (tla--prefix-add    ?t) 'tla-bookmarks-add-tree-interactive)
    (define-key map (tla--prefix-remove ?t) 'tla-bookmarks-delete-tree-interactive)
    (define-key map (tla--prefix-add    ?g) 'tla-bookmarks-add-group-interactive)
    (define-key map (tla--prefix-remove ?g) 'tla-bookmarks-delete-group-interactive)
    (define-key map (tla--prefix-add    ?n) 'tla-bookmarks-add-nickname-interactive)
    (define-key map (tla--prefix-remove ?n) 'tla-bookmarks-delete-nickname-interactive)
...
(defvar tla-category-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map tla--keyvec-add-bookmark 'tla-category-bookmarks-add)
    (define-key map (tla--prefix-apply-from-here
                     tla--key-add-bookmark) 'tla-category-bookmarks-add-here)
...
(defvar tla-branch-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map tla--keyvec-quit 'tla-buffer-quit)
    (define-key map (tla--prefix-apply-from-here
                     tla--key-add-bookmark) 'tla-branch-bookmarks-add-here)
    (define-key map tla--keyvec-add-bookmark 'tla-branch-bookmarks-add)
    map)
...
(defvar tla-version-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map tla--keyvec-quit 'tla-buffer-quit)
    (define-key map (tla--prefix-apply-from-here
                     tla--key-add-bookmark) 'tla-version-bookmarks-add-here)
    (define-key map tla--keyvec-add-bookmark 'tla-version-bookmarks-add)
...
(defvar tla-revision-list-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    ;; from here
    (define-key map (tla--prefix-apply-from-here
                     tla--key-star-merge) 'tla-revision-star-merge)
    ;; from head
    (define-key map (tla--prefix-apply-from-head tla--key-star-merge)
      'tla-revision-star-merge-version)
    (define-key map (tla--prefix-merge tla--key-star-merge)
      'tla-revision-star-merge-version)
    (define-key map tla--keyvec-star-merge 'tla-revision-star-merge-version)

    ;;
    ;; Replay
    ;; from here
    (define-key map (tla--prefix-apply-from-here tla--key-replay)
      'tla-revision-replay)
    ;; from head
    (define-key map (tla--prefix-apply-from-head tla--key-replay)
      'tla-revision-replay-version)
    (define-key map tla--keyvec-replay 'tla-revision-replay-version)

    (define-key map (tla--prefix-apply-from-here ?y)
      'tla-revision-sync-tree)
    ;;
    ;; Update
    (define-key map (tla--prefix-merge tla--key-update)
      'tla-revision-update)
    ...
    ;; from here
    (define-key map (tla--prefix-apply-from-here
                     tla--key-tag) 'tla-revision-tag-from-here)

    ;; from head
    (define-key map (tla--prefix-apply-from-head
                     tla--key-tag) 'tla-revision-tag-from-head)
    (define-key map (tla--prefix-tag
                     tla--key-apply-from-head) 'tla-revision-tag-from-head)
