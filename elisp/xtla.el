  ...
  (interactive
   (if (and (not current-prefix-arg)
     ...
     (cons default-directory
           (tla--read-revision-with-default-tree "Undo against archive: "
                                                 default-directory))))
...
and `tla-replay'."
  (list (tla--name-construct
         (tla-name-read "Relay version or revision: "
                         'prompt 'prompt 'prompt 'prompt 'maybe))
        (tla--read-directory-name "Replay in tree: ")
        current-prefix-arg))
...
(defun tla-replay-reverse (from &optional to-tree arg)
  ...
  (tla-replay from to-tree arg t))


(defun tla-replay (from &optional to-tree arg reverse)
   ...
   (list (tla--name-construct
         ...
         (tla--name-construct
          (tla-name-read "Tag version: " 'prompt 'prompt 'prompt
                         'prompt))))
  (tla--run-tla-async (list "tag" "--setup"
               ...
               `(lambda (output error status arguments)
                  ...
                  (with-current-buffer ,(ewoc-buffer cookie)
                    ...
                    (when (and ',callback
                      ...
                      (funcall ',callback)))
                  (let* ((cur (ewoc-locate tla-revision-list-cookie)))
                    (ewoc-refresh ,cookie)
...
is replayed."
  ...
  (let* ((last-inventory (tla--last-visited-inventory-buffer))
    ...
    (when (and current
               ...
               (eq (caddr (ewoc-data current)) 'bookmark))
      (setq local-tree (cadddr (ewoc-data current))))
    (let ((to-tree (tla--read-directory-name "Replay to tree: " local-tree)))
      (if (tla-revision-marked-revisions)
          (let ((revisions (mapcar 'tla--revision-revision
                            (tla-revision-marked-revisions))))
...
(defun tla-gnus-apply-patch (handle)
  ...
  (let ((archive-name (tla--make-temp-name "gnus-patch-tgz"))
    ...
    (split-window-vertically)
    (tla-show-changeset-from-tgz archive-name)
    (setq tree (tla--read-directory-name
                "Apply to tree: "
